...
public class EFEventsController(IEFCoreService service, ILogger<EFEventsController> logger) : ControllerBase
{
    ...
    // http://localhost:5216/api/EFEvents?pageSize=50&lastId=0
    public async Task<IActionResult> GetEventRegistrations([FromQuery] int pageSize = 10, [FromQuery] int lastId = 0)
    {
        try
        {
            ...
            var pagedResult = await service.GetEventRegistrationsAsync(pageSize, lastId, urlHelper: Url);
    ...
    public async Task<IActionResult> GetEventRegistrationById(int id)
    {
        ...
        try
        {
            var eventR = await service.GetEventRegistrationByIdAsync(id);
